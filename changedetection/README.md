...
This is a single-user application. It provides a single layer of authentication based on the password alone. Once the application has been configured, notifications can be received (e.g. via email, Gotify, Telegram, etc.) without needing access to the application settings. It is therefore probably not necessarily to expose this application to the public internet, if remote access via a VPN is feasible. However, if HTTPS access is needed, the application can be deployed behind a reverse proxy with HTTP basic authentication and fail2ban to thwart brute-force attacks.
